...
class GraphNodeJob(BaseModel):
    ...
    id: str
    job_name: str
    macro_community_id: int = 0  # 第一次聚类算法分的宏观行业社区标签
    micro_community_id: int = 0  # 第二次聚类算法分的细分职业社区标签
    ...
    def transform_job_to_graph_node(job: JDAnalysisResult) -> "GraphNodeJob":
        ...
        job_node = {
            ...
            # --- 2. 物理过滤属性 (全部数字化/序数化，用于 Cypher WHERE) ---
            # 转为数字，方便图库里做 >= 的大小比较
            "min_degree": degree_map.get(job_profiles.basic_requirements.degree.value, 0),
            "salary_rank": level_map.get(job_profiles.job_attributes.salary_competitiveness.value, 2),
            "demand_rank": level_map.get(job_profiles.job_attributes.social_demand.value, 2),
            "trend_rank": trend_map.get(job_profiles.job_attributes.industry_trend.value, 0),
    ...
    def transform_competencies_to_graph_nodes(
        ...
        for category, items in source_map.items():
            ...
            for item in items:
                ...
                if clean_name and clean_name.lower() not in {"无", "无要求", "未提及", "不限"}:
                    competencies.append(
