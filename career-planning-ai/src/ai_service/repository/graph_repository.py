...
class GraphRepository:
    ...
    def build_graph_from_jobs(
        ...
        # 说明：由于社区 ID 是“标签”而非语义，稳定性指标采用 ARI/NMI（对标签置换不敏感）。
        prev_macro_labels = {
            jid: int(job_data[jid].get("macro_community_id", -1)) for jid in job_data
        }
        prev_micro_labels = {
            jid: int(job_data[jid].get("micro_community_id", -1)) for jid in job_data
        }
        ...
        clustering_data = {
            ...
            "fine_resolution": float(settings.graph_build.clustering.fine_resolution),
        ...
        # 展示帕累托筛选后的边数和之前的对比（注意：帕累托是在“有向候选边”上做的）
        log.info(
        ...
        # 6.1 跨社区保边兜底：确保“存在候选跨社区边的社区对”至少保留 1 条跨社区边
        # 说明：帕累托剪枝对跨社区边施加惩罚维度，可能导致某些社区对的跨社区边全部被剪掉。
        # 这里用最低成本(最易走)的跨社区候选边补回，避免社区之间完全断联。
        ...
        best_cross_edge_by_pair: dict[tuple[int, int], tuple[float, str, str, dict]] = (
            {}
        )
        ...
        # 7. 用 final_routing_cost 和帕累托剪枝后的边跑第二次细聚类
        # P0 修复：细聚类是无向聚类，但输入为“有向边”。如果直接把有向 dict 传入，
        # run_clustering 会按无向去重且受 dict 迭代顺序影响，导致结果漂移。
        # 这里显式聚合成无向对，并取两向中更“强”的连接（成本更低）作为无向权重。
        ...
        # =====================================================
        # BuildRun 核心报表指标（用于迭代回归）
        # - 社区稳定性：ARI/NMI（与上一轮标签对比，置换不敏感）
        # - 连通性：无向连通分量、最大连通分量占比、孤点比例
        # - 跨社区覆盖率：候选跨社区对中，最终至少保留 1 条边的比例
        # - 路径可达率：在有向 EVOLVE 图中，<=2 hop 是否可达跨界/跨赛道目标
        # =====================================================
        report: Dict[str, Any] = {}
        try:
            from collections import defaultdict
            ...
            undirected_degree = defaultdict(int)
            for a, b in pareto_skeleton_edges.keys():
            ...
            # 3) 跨社区边覆盖率（候选跨社区社区对 -> 最终保留至少 1 条边）
            candidate_macro_pairs: set[tuple[int, int]] = set()
            candidate_micro_pairs: set[tuple[int, int]] = set()
            for a, b in edge_info_map.keys():
                ca0 = int(coarse_community_map.get(a, -1))
                cb0 = int(coarse_community_map.get(b, -1))
                if ca0 != cb0 and ca0 >= 0 and cb0 >= 0:
                    candidate_macro_pairs.add(tuple(sorted((ca0, cb0))))

                ca1 = int(fine_community_map.get(a, -1))
                cb1 = int(fine_community_map.get(b, -1))
            ...
            for (a, b), m in pareto_skeleton_edges.items():
                mac_a = int(job_data[a].get("macro_community_id", -1))
                mac_b = int(job_data[b].get("macro_community_id", -1))
                mic_a = int(job_data[a].get("micro_community_id", -1))
                mic_b = int(job_data[b].get("micro_community_id", -1))
            ...
            # 大图时做抽样，避免离线构建时间被报表拖垮
            if len(sample_nodes) > 5000:
                import random

                rng = random.Random(42)
                sample_nodes = rng.sample(sample_nodes, k=min(2000, len(sample_nodes)))
            ...
            reachability = {
                ...
                "any_reachable_rate": round(
                    (any_reach / len(sample_nodes)) if sample_nodes else 0.0, 6
                ),
                "cross_macro_reachable_rate": round(
                    (cross_macro_reach / len(sample_nodes)) if sample_nodes else 0.0, 6
                ),
                "cross_micro_reachable_rate": round(
                    (cross_micro_reach / len(sample_nodes)) if sample_nodes else 0.0, 6
                ),
            }
