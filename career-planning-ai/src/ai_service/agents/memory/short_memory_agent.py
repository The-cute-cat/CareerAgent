...
class ShortMemoryAgent:
    ...
    def _build_key(session_id: str) -> str:
        """构建 Redis 键名"""
        return f"short_memory:{session_id}"
    ...
    async def add_message(
        ...
        if auto_compress and self.should_compress(messages):
            logger.info(f"会话 {session_id} 触发压缩条件，开始压缩...")
            messages = await self.compress_messages(session_id, messages)
        key = self._build_key(session_id)
        messages_data = [msg.model_dump() for msg in messages]
        self.redis.set(key, messages_data, ttl=None)
        logger.debug(f"会话 {session_id} 添加消息，当前消息数: {len(messages)}")
        return messages
    ...
    async def compress_messages(
        ...
        logger.info(
            f"会话 {session_id} 压缩完成: {len(messages)} -> {len(compressed_messages)} 条消息"
        )
