...
const form = reactive<LoginFormDTO>({})
...
const sendVerificationCode = async () => {
  ...
  try {
    // 模拟API调用发送验证码
    console.log('发送验证码至邮箱:', form.email)
    await new Promise(resolve => setTimeout(resolve, 1000))

    alert(`验证码已发送至 ${form.email} (演示模式，请检查控制台)`)
                ...
                <div class="form-group mb-3">
                  ...
                  <div class="position-relative">
                    <input :type="showConfirmPassword ? 'text' : 'password'" class="form-control" placeholder="请再次输入密码"
                      id="password_confirm" v-model="form.confirmPassword" required />
                ...
                <div class="form-group mb-3">
                  ...
                  <div class="d-flex align-items-center gap-2" style="gap: 8px;">
                    <input type="text" class="form-control" placeholder="请输入邮箱收到的验证码" id="verification_code"
                      v-model="form.verificationCode" required style="flex: 1;" />
