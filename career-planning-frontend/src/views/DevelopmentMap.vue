    ...
    <section class="content-grid">
      ...
      <aside class="detail-column">
        ...
        <div v-if="viewMode === 'vertical'" class="detail-card">
          ...
          <el-timeline>
            <el-timeline-item
              v-for="step in selectedPromotionTrail"
              :key="step.id"
              :type="step.id === selectedJobId ? 'primary' : 'success'"
          ...
          <el-alert :title="`下一阶段目标：${selectedJob.nextStep}`" :description="selectedJob.nextAdvice" type="success" :closable="false" show-icon />
        </div>
        ...
        <div v-else class="detail-card">
          ...
          <div class="transfer-role-list">
            <div
              v-for="role in transferRoles"
              :key="role.id"
              class="transfer-role-item"
...
import { buildDynamicDevelopmentData, loadCareerFormData, loadJobMatchResult } from '@/utils/career-runtime'
...
const coreSkills = computed(() => splitText(topMatch.value?.raw_data.profiles.professional_skills.core_skills))
const promotionPath = computed(() => {
  const path = splitText(topMatch.value?.raw_data.profiles.job_attributes.vertical_promotion_path)
  return path.length ? path : [`资深${runtimeMeta.value.targetJobName}`, `${runtimeMeta.value.targetJobName}负责人`, `高级${runtimeMeta.value.targetJobName}管理岗`]
})
...
const promotionNodes = computed<PromotionNode[]>(() => {
  ...
  const baseSkills = coreSkills.value.length ? coreSkills.value : ['岗位核心技能', '项目协作', '业务理解']
  ...
  return stages.map((name, index) => ({
    ...
    goal: index === 0 ? `进入 ${name} 岗位并完成岗位基础能力补齐。` : `向 ${name} 发展，提升更复杂场景下的项目和业务能力。`,
    summary: index === 0 ? `结合职业画像与匹配结果，当前优先关注 ${name}。` : `${name} 是 ${targetJob} 的后续发展方向之一。`,
    description: topMatch.value?.deep_analysis.all_analysis || `围绕 ${name} 的岗位要求，持续提升专业能力、业务理解与项目产出。`,
    deliverables: ['项目成果', '方案沉淀', '阶段复盘'],
    skills: baseSkills.slice(0, Math.min(baseSkills.length, 4)),
    nextStep: stages[index + 1] || '持续成长',
    nextAdvice: index === 0 ? (topMatch.value?.deep_analysis.actionable_advice || '优先补齐关键技能差距，进入目标岗位。') : `继续向 ${stages[index + 1] || '更高层级岗位'} 提升。`
  }))
})
...
const transferRoles = computed<TransferRole[]>(() => lateralDirections.value.slice(0, 5).map(buildTransferPath))
const promotionTrail = computed(() => promotionNodes.value.map(({ id, name, period, goal }) => ({ id, name, period, goal })))
const totalTransferPaths = computed(() => transferRoles.value.reduce((sum, role) => sum + role.paths.length, 0))
...
const selectedPromotionTrail = computed(() => {
  ...
  return currentIndex < 0 ? promotionTrail.value : promotionTrail.value.slice(0, Math.min(currentIndex + 2, promotionTrail.value.length))
})

const selectedTransferRole = computed(
  () => transferRoles.value.find((item) => item.id === selectedTransferRoleId.value) ?? transferRoles.value[0] ?? emptyTransferRole
)

const currentViewMeta = computed(() =>
  viewMode.value === 'vertical'
    ? { kicker: '岗位晋升关系', title: `垂直岗位图谱：${runtimeMeta.value.targetJobName} 的成长路径` }
    : { kicker: '岗位血缘关系', title: `换岗路径图谱：${runtimeMeta.value.targetJobName} 的横向转岗方向` }
)
  ...
  series: [
    {
      ...
      lineStyle: { width: 3, color: '#1f8fff', curveness: 0.18 },
      ...
      links: promotionNodes.value.slice(0, -1).map((node, index) => {
        ...
        return {
          source: node.id,
          target: nextNode?.id ?? '',
          label: { show: true, formatter: nextNode?.period ?? '' }
        }
      })
    }
  ]
  ...
  series: [
    {
      ...
      roam: true,
      zoom: currentZoom.value,
      force: { repulsion: 320, edgeLength: [120, 160] },
      label: { show: true, formatter: '{b}', fontSize: 12 },
      lineStyle: { color: '#fb7185', width: 2, type: 'dashed', curveness: 0.15 },
      data: [
        {
          id: 'origin',
          name: runtimeMeta.value.targetJobName,
          symbolSize: 94,
          itemStyle: { color: '#f97316', borderColor: '#7c2d12', borderWidth: 2 }
        },
      ...
      links: transferRoles.value.flatMap((role) =>
        role.paths.map((path) => ({ source: 'origin', target: role.id, value: path.title, label: { show: false } }))
      )
    }
  ]
})
...
.hero-card,.chart-card,.summary-card,.detail-card { background: rgba(255, 255, 255, 0.9); border: 1px solid rgba(148, 163, 184, 0.18); box-shadow: 0 16px 40px rgba(15, 23, 42, 0.08); backdrop-filter: blur(10px); }
.hero-card { display: grid; grid-template-columns: 1.35fr 1fr; gap: 28px; padding: 28px; border-radius: 28px; }
.eyebrow,.card-kicker { display: inline-flex; align-items: center; font-size: 12px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: #2563eb; }
.hero-copy h1 { margin: 12px 0 14px; font-size: 34px; line-height: 1.25; color: #0f172a; }
.hero-copy p { margin: 0; font-size: 15px; line-height: 1.8; color: #475569; }
.hero-actions { display: flex; align-items: center; }
.job-selector { width: 220px; }
.hero-stat-list { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px; }
.hero-stat { padding: 16px; border-radius: 20px; background: linear-gradient(135deg, #dbeafe 0%, #fef3c7 100%); }
.hero-stat strong { display: block; margin-bottom: 6px; font-size: 28px; color: #0f172a; }
.hero-stat span { font-size: 13px; color: #475569; }
.content-grid { display: grid; grid-template-columns: minmax(0, 1.5fr) 430px; gap: 22px; margin-top: 22px; }
.chart-card { padding: 24px; border-radius: 26px; }
.card-head,.section-title,.transfer-path-top { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
.card-head { align-items: flex-start; }
.chart-head-left { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
.chart-head-right { text-align: right; }
.chart-toolbar { display: flex; align-items: center; justify-content: space-between; gap: 16px; margin-top: 18px; padding: 14px 16px; border-radius: 18px; background: linear-gradient(135deg, rgba(219, 234, 254, 0.78), rgba(255, 247, 237, 0.78)); }
.toolbar-note { font-size: 13px; color: #475569; }
.card-head h2,.section-title h3,.summary-card h3 { margin: 6px 0 0; color: #0f172a; }
.graph-canvas { height: 620px; margin-top: 16px; }
.legend-row { display: flex; flex-wrap: wrap; gap: 18px; padding-top: 8px; }
...
.legend-dot.current { background: #1d4ed8; }
.legend-dot.next { background: #f97316; }
.legend-dot.related { background: #a855f7; }
.legend-line { width: 26px; height: 2px; background: #475569; }
.legend-line.dashed { background: repeating-linear-gradient(to right,#475569 0,#475569 6px,transparent 6px,transparent 12px); }
.detail-column { display: flex; flex-direction: column; gap: 16px; }
.summary-card,.detail-card { padding: 22px; border-radius: 24px; }
.summary-card p,.detail-card p,.transfer-role-item p { color: #475569; line-height: 1.7; }
.summary-tags { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 14px; }
.path-block p { margin: 6px 0 0; }
.transfer-role-list { display: flex; flex-direction: column; gap: 10px; }
.transfer-role-item { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; padding: 14px 16px; border-radius: 18px; background: #f8fafc; border: 1px solid transparent; cursor: pointer; transition: all 0.2s ease; }
.transfer-role-item.active,.transfer-role-item:hover { border-color: rgba(236, 72, 153, 0.4); background: #fff1f7; transform: translateY(-1px); }
.transfer-role-item span { color: #be185d; font-size: 12px; white-space: nowrap; }
.transfer-path-card { padding: 16px; border-radius: 18px; background: linear-gradient(135deg, #fff7ed 0%, #fdf2f8 100%); }
.transfer-path-card + .transfer-path-card { margin-top: 12px; }
.mini-label { margin-top: 10px; font-size: 12px; font-weight: 700; color: #7c3aed; }
.plain-steps { margin: 8px 0 0; padding-left: 18px; color: #475569; line-height: 1.8; }
@media (max-width: 1200px) { .hero-card,.content-grid { grid-template-columns: 1fr; } .detail-column { order: 2; } }
@media (max-width: 768px) { .development-map-page { padding: 14px; } .hero-card,.chart-card,.summary-card,.detail-card { border-radius: 20px; } .hero-copy h1 { font-size: 26px; } .hero-stat-list,.content-grid { grid-template-columns: 1fr; } .graph-canvas { height: 480px; } .card-head,.section-title,.transfer-path-top,.chart-toolbar,.chart-head-left { align-items: flex-start; flex-direction: column; } .chart-head-right { text-align: left; } }
</style>
